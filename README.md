...
- Создана сеть и подсети в 3-х зонах доступности, публичная для мастера, приватная для рабочих узлов:
...
До этого был поднят selfhost GitLab server. Были настроены ранеры для сборки Docker образов и раннер для деплоя приложения в кластер. Также все секретные данные были размещены в переменных GitLab.

Инфраструктура по большей части получилась по методике `IAC Infrastructure as code`.
После отправки дипломной работы на проверку, вся инфраструктура будет удалена, и по необходимости поднята в короткие сроки заново!
